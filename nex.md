...
1. Each PRUDP packet requires its own RC4 stream. If the stream isn't reset between packets, the data gets encrypted differently and the console is unable to read the packet. While **SuperMarioDaBom** hasn't observed this with every RC4 library, he has seen it with the implementation that nex-go uses.
